...
export const filterProjects = async (
	...
	const filtered = allProjects.filter((project) => {
		...
		const descriptionMatch = project.excerpt
			?.toLowerCase()
		...
		const searchCondition =
			nameMatch || descriptionMatch || tagMatch || clientMatch;
...
export const getProject = async (slug: string, locale: Locale) => {
	...
	const availableProjects = getContentLocales(
		slugMetadata?.uniqueId || "",
		"projects",
	);
	const metadata = availableProjects.find(
	...
	if (slugMetadata?.locale !== locale) {
		return redirect(
			302,
			localizeHref(route("/projets/[slug]", { slug: hrefSlug }), { locale }),
		);
	}
